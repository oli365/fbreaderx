...
public final class FBView extends ZLTextView {
    ...
    public void onFingerSingleTap(int x, int y) {
        ...
        final ZLTextRegion imageRegion = findRegion(x, y, 0, ZLTextRegion.ImageOrHyperlinkFilter);
        if (imageRegion != null) {
            outlineRegion(imageRegion);
            myReader.getViewWidget().reset();
            myReader.getViewWidget().repaint();
            myReader.runAction(ActionCode.PROCESS_HYPERLINK);
            return;
...
//                        }
    ...
    public synchronized void onScrollingFinished(PageIndex pageIndex) {
        ...
        ZLApplication.Instance().runAction(ActionCode.PAGE_TURNING_END);
    }
